...
__global__ void createBevMap(float4* points, float* bev, int points_size, 
                             float* point_range, int* size) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < points_size) {
        float4 point = points[i];
        float x_scale_factor = size[0] / (point_range[3] - point_range[0]);
        float y_scale_factor = size[1] / (point_range[4] - point_range[1]);

        int x_index = static_cast<int>((point.x - point_range[0]) * x_scale_factor);
        int y_index = static_cast<int>((point.y - point_range[1]) * y_scale_factor);
        if (x_index >= 0 && x_index < size[0] && y_index >= 0 && y_index < size[1]) {
            int base_index = y_index * size[0] * 2 + x_index * 2;
            bev[base_index] = (bev[base_index]>point.z?bev[base_index]:point.z); // 更新z值
            bev[base_index+1]= (bev[base_index+1]>point.w?bev[base_index+1]:point.w); // 更新反射率
        }
    }
}
...
cudaError_t createBevMap_launch(float4* points, float* bev, int points_size, 
                             float* point_range, int* size,cudaStream_t stream)
{
    dim3 threads={32,32};
    dim3 blocks={(1408 + threads.x -1)/threads.x,
                 (1600 + threads.y -1)/threads.y};
    createBevMap<<<blocks, threads,0,stream>>>(points, bev, points_size, point_range, size);
    cudaError_t err = cudaGetLastError();
    return err;
}