...
BEVDet::BEVDet(std::string modelFile, cudaStream_t stream):stream_(stream)
{
  checkCudaErrors(cudaEventCreate(&start_));
  checkCudaErrors(cudaEventCreate(&stop_));
...
int BEVDet::doinfer(float*points_data, std::vector<Bndbox> &nms_pred)
{
  ...
  void *buffers[] = {points_data,cls_output_, box_output_,dir_cls_output_};
  ...
  post_->doPostprocessCuda(cls_output_, box_output_,
                          bndbox_output_);
                          
  // std::vector<float> hostData(  params_.feature_x_size * params_.feature_y_size * params_.num_box_values * params_.num_anchors * sizeof(float));
  // cudaMemcpy(hostData.data(),  box_output_,  params_.feature_x_size * params_.feature_y_size * params_.num_box_values * params_.num_anchors * sizeof(float), cudaMemcpyDeviceToHost);
  ...
  // auto output = hostData.data();
  // for(int i=0;i<num_obj;i++){
  //   std::cout<<"box_output: "<<output[i * 7]<<" "<<output[i * 7 + 1]<<" "<<output[i * 7 + 2]<<" "<<output[i * 7 + 3]<<" "<<output[i * 7 + 4]<<" "<<output[i * 7 + 5]<<" "<<output[i *7 + 6]<<" "<<std::endl;
  // }
  for (int i = 0; i < num_obj; i++) {
    auto Bb = Bndbox(output[i * 9],
                    ...
                    std::cout<<"box_output: "<<output[i * 9]<<" "<<output[i * 9 + 1]<<" "<<output[i * 9 + 2]<<" "<<output[i * 9 + 3]<<" "<<output[i * 9 + 4]<<" "<<output[i * 9 + 5]<<" "<<output[i *9 + 6]<<" "<<std::endl;
