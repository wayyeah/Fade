...
int PreProcessCuda::generateBEV(float4* points, float* bev, int points_size, 
                             float* point_range, int* size){
  
  checkCudaErrors( createBevMap_launch(points, bev, points_size, point_range, size,stream_));
  return 0;
}
