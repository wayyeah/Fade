...
class SigmoidFocalIouLoss(nn.Module):
    def __init__(self):
        super(SigmoidFocalIouLoss, self).__init__()
        # self.high_thre = high_thre
        # self.low_thre = low_thre

    def forward(self, input: torch.Tensor, target: torch.Tensor, weights):
        """
        Args:
            input: (B, #anchors, #classes) float tensor.
                Predicted logits for each class
        ...
        Returns:
            weighted_loss: (B, #anchors, #classes) float tensor after weighting.
        """
        # target_new = (target >= self.high_thre).float() + ((target < self.high_thre) & (target > self.low_thre)).float() * (
        #         (target - self.low_thre) / (self.high_thre - self.low_thre)) #  4-5

        # target_new = (target >= self.high_thre).float() # 6-9
        # bce_loss = F.mse_loss(torch.sigmoid(input), target_new, reduction='none')

        bce_loss = F.mse_loss(torch.sigmoid(input), target, reduction='none') # 10, 11, 12
        focal_weight = 1.0
...
class IOULossCenterNet(nn.Module):
    ...
    def forward(self, out, target, weights=None):
        ...
        bce_loss = F.mse_loss(out, target, reduction='none')
        bce_loss = bce_loss.view(out.shape[0], out.shape[1], -1)
