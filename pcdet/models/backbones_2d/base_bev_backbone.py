...
class BaseBEVBackbone_flexible(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        num_levels = len(layer_nums)
        num_filters_in_select = []
        select_filter_tmp = 0
        for i in layer_nums[:-1]:
            num_filters_in_select.append(num_filters[select_filter_tmp+i])
            select_filter_tmp = select_filter_tmp + i + 1
        c_in_list = [input_channels, *num_filters_in_select]
        layer_nums_tmp = [0] + [i + 1 for i in layer_nums]

        deconv_output_num = num_filters[sum(layer_nums_tmp):]

        self.blocks = nn.ModuleList()
        self.deblocks = nn.ModuleList()
        for idx in range(num_levels):

            cur_num_filter = num_filters[(sum(layer_nums_tmp[:(idx+1)])):(sum(layer_nums_tmp[:(idx+2)]))]
            ...
            cur_layers = [
                ...
                nn.Conv2d(
                    c_in_list[idx]*4 if self.model_cfg.get('FOCUS', None) and layer_strides[idx] > 1 else c_in_list[idx],
                    cur_num_filter[0], kernel_size=3,
                    stride=layer_strides[idx] if not self.model_cfg.get('FOCUS', None) else 1,
                    padding=1, bias=False
                ),
                norm_layer(cur_num_filter[0], eps=1e-3, momentum=0.01),
                act_fn()
            ]
            for k in range(layer_nums[idx]):
                cur_layers.extend(
                    build_block(
                        self.model_cfg.get('CONV_BLOCK', 'BasicBlock2D'), cur_num_filter[k],
                        cur_num_filter[k+1], act_fn=act_fn, kernel_size=3, padding=1, bias=False,
                        norm_layer=norm_layer
                    ),
                )
            self.blocks.append(nn.Sequential(*cur_layers))
            if len(upsample_strides) > 0:
                ...
                if stride >= 1:
                    self.deblocks.append(
                        build_deconv_block(
                            self.model_cfg.get('DECONV_BLOCK', 'ConvTranspose2dBlock'),
                            cur_num_filter[-1], deconv_output_num[idx],
                            kernel_size=upsample_strides[idx],
                            stride=upsample_strides[idx], bias=False,
                            act_fn=act_fn,
                            norm_layer=norm_layer
                    ...
                    self.deblocks.append(nn.Sequential(
                        nn.Conv2d(
                            num_filters[idx], deconv_output_num[idx],
                            kernel_size=stride,
                            stride=stride, bias=False
                        ),
                        nn.BatchNorm2d(deconv_output_num[idx], eps=1e-3, momentum=0.01),
                        act_fn()
                    ))

        c_in = sum(deconv_output_num)
        if len(upsample_strides) > num_levels:
            self.deblocks.append(
                build_deconv_block(
                    self.model_cfg.get('DECONV_BLOCK', 'ConvTranspose2dBlock'),
...
class BaseBEVBackboneCas(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.num_frames=1
        self.model_cfg = model_cfg
    ...
    def forward(self, data_dict):
        ...
        for j in range(self.num_frames):
            if j==0:
                if 'temporal_features' in data_dict:
                    spatial_features = data_dict['temporal_features']
                else:
