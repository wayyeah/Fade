...
def draw_gaussian_to_heatmap(heatmap, center, radius, k=1, valid_mask=None, sharper=False):
    diameter = 2 * radius + 1
    gaussian = gaussian2D((diameter, diameter), sigma=diameter / 6)
    if sharper:
        new_gaussian = gaussian.copy()
        g_c = radius
        dx = [0, 0, 1, -1]
        dy = [1, -1, 0, 0]
        height, width = gaussian.shape[0:2]
        for i in range(len(dx)):
            sub_gaussian = np.zeros(gaussian.shape, dtype=np.float32)
            sub_x, sub_y = g_c + dx[i], g_c + dy[i]
            left, right = min(sub_x, radius), min(width - sub_x, radius + 1)
            top, bottom = min(sub_y, radius), min(height - sub_y, radius + 1)

            sub_gaussian[sub_y - top:sub_y + bottom, sub_x - left:sub_x + right] = gaussian[g_c - top:g_c + bottom, g_c - left:g_c + right]
            new_gaussian = np.fmax(new_gaussian, sub_gaussian)
