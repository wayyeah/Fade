    ...
    def forward(self, batch_dict):
        ...
        for i in range(self.stages):
            ...
            if not self.training :
                ...
                self.forward_ret_dict['targets_dict'+stage_id] = targets_dict
            if not batch_dict.get('re_run_tea_flag', None):
                targets_dict['rcnn_cls'] = rcnn_cls
                targets_dict['rcnn_reg'] = rcnn_reg
        ...
        if not self.training:
            batch_dict['batch_box_preds'] = torch.mean(torch.stack(all_preds), 0)
            batch_dict['batch_cls_preds'] = torch.mean(torch.stack(all_scores), 0)
            batch_dict['rcnn_cls_stu_to_tea'] = torch.mean(torch.stack(all_scores), 0)
            batch_dict['rcnn_reg_stu_to_tea'] = torch.mean(torch.stack(all_preds), 0)
        return batch_dict
