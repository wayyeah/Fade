...
class AnchorFeatureKDHead(KDHeadTemplate):
    ...
    def build_feature_kd_loss(self):
        ...
        if 'use_reg' in self.model_cfg.KD_LOSS.FEATURE_LOSS:
            if self.model_cfg.KD_LOSS.FEATURE_LOSS.use_reg and (self.model_cfg.KD_LOSS.FEATURE_LOSS.reg_loss_type in ['SmoothL1Loss']):
                self.kd_feature_reg_loss_func = getattr(torch.nn, self.model_cfg.KD_LOSS.FEATURE_LOSS.reg_loss_type)(reduction='none')
    ...
    def get_feature_kd_loss(self, batch_dict, tb_dict, loss_cfg):
        ...
        elif loss_cfg.mode == 'point':
            kd_feature_loss = self.get_feature_kd_loss_point(batch_dict, loss_cfg)
        else:
            raise NotImplementedError
    ...
    def get_feature_kd_loss_point(self, batch_dict, loss_cfg):

        select_mask = batch_dict['select_mask'].view(-1, 1)
        cls_preds_kd_tea = batch_dict['cls_preds_kd_tea']
        cls_preds_kd_stu = batch_dict['cls_preds_kd_stu']

        if loss_cfg.get('use_sigmoid', None) and loss_cfg.use_sigmoid:
            cls_preds_kd_tea = torch.sigmoid(cls_preds_kd_tea) * select_mask
            cls_preds_kd_stu = torch.sigmoid(cls_preds_kd_stu) * select_mask

        # if loss_cfg.get('use_reg', None) and loss_cfg.use_reg:
        #     box_preds_kd_tea = batch_dict['box_preds_kd_tea'] * select_mask
        #     box_preds_kd_stu = batch_dict['box_preds_kd_stu']* select_mask

        # if 'use_reg' in loss_cfg:
        #     if loss_cfg.use_reg:
        #         kd_feature_loss = loss_cfg.weight_reverse * \
        #                           (
        #                               (self.kd_feature_loss_func(cls_preds_kd_stu, cls_preds_kd_tea).sum() / torch.clamp(
        #                               select_mask.sum(), min=1.0)) +
        #                               (self.kd_feature_reg_loss_func(box_preds_kd_stu, box_preds_kd_tea).sum() / torch.clamp(
        #                                   select_mask.sum(), min=1.0))
        #                           )
        # else:
        kd_feature_loss = loss_cfg.weight_reverse * \
