...
class AnchorLogitKDHead(KDHeadTemplate):
    ...
    def cal_anchor_weight(self, box_pred_tea):
        box_reg_targets = self.dense_head.forward_ret_dict['box_reg_targets']
        box_cls_labels = self.dense_head.forward_ret_dict['box_cls_labels']
        batch_size = int(box_pred_tea.shape[0])
        ...
        box_preds_tea = box_pred_tea.view(batch_size, -1,
                                           box_pred_tea.shape[-1] // self.dense_head.num_anchors_per_location)

        box_preds_sin, reg_targets_sin = self.add_sin_difference(box_preds_tea, box_reg_targets)
        loc_loss_src = self.kd_reg_loss_func(box_preds_sin, reg_targets_sin, weights=reg_weights)  # [N, M]
        loc_loss_sum = loc_loss_src.sum(-1)
        loc_loss_exp = torch.sigmoid(1e5*loc_loss_sum)
        # loc_loss_exp = loc_loss_sum
        loc_loss_exp_max = loc_loss_exp.max(-1)[0].unsqueeze(-1).repeat(1, loc_loss_exp.shape[1])
        loc_loss_exp_norm = (loc_loss_exp - 0.5) / (loc_loss_exp_max - 0.5)
        # loc_loss_exp_norm = (loc_loss_exp) / (loc_loss_exp_max)
        weight = loc_loss_exp_norm
        weight = weight * positives.float()
    ...
    def cal_kd_reg_loss(self, box_preds_stu, box_preds_tea):
        #add
        # weight_reg_obj = self.cal_anchor_weight(box_preds_tea)
